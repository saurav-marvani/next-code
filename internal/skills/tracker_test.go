...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (nextcode-config).
	activeSkills := []*Skill{
		{Name: "nextcode-config", Description: "NextCode config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("nextcode-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via nextcode://...).
	tracker.MarkLoaded("nextcode-config")
	require.True(t, tracker.IsLoaded("nextcode-config"))
