...
func TestProviders_Integration_WithMockClients(t *testing.T) {
	...
	catwalkPath := tmpDir + "/nextcode/providers.json"
	hyperPath := tmpDir + "/nextcode/hyper.json"
...
func TestProviders_Integration_WithCachedData(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(tmpDir+"/nextcode", 0o755))
...
func TestProviders_Integration_BothFail(t *testing.T) {
	...
	hyperResult, err := testHyperSyncer.Get(t.Context())
	require.NoError(t, err)
	require.Equal(t, "NextCode Hyper", hyperResult.Name) // Falls back to embedded when no models.
}
...
func TestCachePathFor(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expected:    "/custom/data/nextcode/providers.json",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expected != "" {
				...
				require.Contains(t, result, "nextcode")
