// Package agent is the core orchestration layer for NextCode AI agents.
//
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `nextcode run` against a
	...
	// event, so non-interactive clients (e.g. `nextcode run`) don't
...
// RunID, e.g. `nextcode run`, would otherwise hang). Uncanceled calls without
...
// that RunID: `nextcode run` ignores live message events and exits only on a
...
// so callers waiting on those RunIDs (e.g. `nextcode run`) are not left
...
// caller waiting on RunComplete (e.g. `nextcode run` with a RunID) always
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if call.Accepted != nil {
		...
		if a.canceledBySeq(call.SessionID, call.Accepted.seq) {
			...
			// RunComplete for this RunID (e.g. `nextcode run`, which
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "nextcode auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("NEXTCODE_DISABLE_ANTHROPIC_CACHE")); t {
