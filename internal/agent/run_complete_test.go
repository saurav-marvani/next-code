...
// event into a closure nobody reads, and subscribers (`nextcode run`)
...
// publish a RunComplete for its RunID, hanging a `nextcode run` caller that
...
// than internal bookkeeping, which is the contract a `nextcode run` caller
...
// bookkeeping slice, not the published event a `nextcode run` caller awaits.
