...
func TestReadBuiltinFile(t *testing.T) {
	...
	t.Run("reads nextcode-config skill", func(t *testing.T) {
		t.Parallel()

		resp, err := readBuiltinFile(ViewParams{
			FilePath: "nextcode://skills/nextcode-config/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.NotEmpty(t, resp.Content)
		require.Contains(t, resp.Content, "NextCode Configuration")
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		resp, err := readBuiltinFile(ViewParams{
			FilePath: "nextcode://skills/nonexistent/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.True(t, resp.IsError)
	})
	...
	t.Run("metadata has skill info", func(t *testing.T) {
		...
		require.Equal(t, "nextcode-config", meta.ResourceName)
