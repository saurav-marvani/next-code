...
nextcode run "Generate a hot README for this project" > MY_HOT_README.md

# Run in quiet mode (hide the spinner)
nextcode run --quiet "Generate a README for this project"

# Run in verbose mode (show logs)
nextcode run --verbose "Generate a README for this project"

# Continue a previous session
nextcode run --session {session-id} "Follow up on your last response"

# Continue the most recent session
nextcode run --continue "Follow up on your last response"
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if useClientServer() {
			...
			if !ws.Config.IsConfigured() {
				return fmt.Errorf("no providers configured - please run 'nextcode' to set up a provider interactively")
			}
...
// streaming SSE events into a final, complete stdout for `nextcode run`.
...
func (s *runStream) handle(ev any, stopSpinner func()) (done bool, err error) {
	...
	case pubsub.Event[proto.RunComplete]:
		...
		// source of the regression where `nextcode run` exited
...
// prefix returned by nextcode session list.
