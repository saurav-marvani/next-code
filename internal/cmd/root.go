...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom nextcode data directory")
	rootCmd.PersistentFlags().BoolP("debug", "d", false, "Debug")
	rootCmd.PersistentFlags().StringVarP(&clientHost, "host", "H", server.DefaultHost(), "Connect to a specific nextcode server host (for advanced users)")
...
cat README.md | nextcode run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
nextcode --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
nextcode --yolo

# Run with custom data directory
nextcode --data-dir /path/to/custom/.nextcode

# Continue a previous session
nextcode --session {session-id}
...
// enabled via the NEXTCODE_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("NEXTCODE_CLIENT_SERVER"))
	return v
}
...
// NEXTCODE_CLIENT_SERVER=1, it connects to a server process and returns a
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	logFile := filepath.Join(cfg.Options.DataDirectory, "logs", "nextcode.log")
	nextcodelog.Setup(logFile, debug)
...
func connectToServer(cmd *cobra.Command) (*client.Client, *proto.Workspace, func(), error) {
	...
	if ws.Config != nil {
		logFile := filepath.Join(ws.Config.Options.DataDirectory, "logs", "nextcode.log")
		nextcodelog.Setup(logFile, debug)
	}
...
func ensureServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	// server log file. nextcodelog.Setup uses sync.Once internally, so the
	// later call from connectToServer becomes a no-op.
	debug, _ := cmd.Flags().GetBool("debug")
	logFile := filepath.Join(config.GlobalCacheDir(), "server-"+safeHostName(hostURL), "nextcode.log")
	nextcodelog.Setup(logFile, debug)
	...
	case "unix", "npipe":
		...
		if needsStart {
			if err := spawnAndWaitReady(cmd, hostURL); err != nil {
				return fmt.Errorf("failed to initialize nextcode server: %v", err)
			}
			return nil
		}
...
// $XDG_CACHE_HOME/nextcode/server-<safeHost>/start.lock.
//
...
// Overridable via NEXTCODE_SERVER_READY_TIMEOUT (parsed as a Go duration).
func serverReadyTimeout() time.Duration {
	const def = 10 * time.Second
	v := os.Getenv("NEXTCODE_SERVER_READY_TIMEOUT")
	if v == "" {
		return def
	}
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start nextcode server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach nextcode server process: %v", err)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("NEXTCODE_DISABLE_METRICS")); v {
		return false
	}
...
func createDotNextCodeDir(dir string) error {
