...
func TestNextCodeIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .nextcodeignore file that ignores .log files
	require.NoError(t, os.WriteFile(".nextcodeignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .nextcodeignore file
	nextcodeignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".nextcodeignore"), []byte(nextcodeignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .nextcodeignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .nextcodeignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .nextcodeignore in subdir that ignores normal_nested
	subNextCodeignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".nextcodeignore"), []byte(subNextCodeignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .nextcodeignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .nextcodeignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .nextcodeignore")
